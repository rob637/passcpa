...
def check_lesson_content(section, l_id, content_obj):
    ...
    for idx, sec in enumerate(sections):
        ...
        s_content = sec.get('content') # Can be string or list (for table rows, etc)
        ...
        if s_type == 'table':
        ...
        elif s_type in ['text', 'callout', 'example', 'list']:
            ...
            if isinstance(s_content, str):
                ...
                # Check for "null" ONLY if it looks like a code error, not SQL
                # Regex for standalone "null" might be better, or just ignore "null" generally as it's common in tech
                # But "undefined" is almost always bad in content
...
if not issues:
    ...
    for i in issues[:50]: # Limit output
        print(f"- {i}")
    if len(issues) > 50:
