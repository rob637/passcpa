 ...
 */
export const getAllTopicsForSection = (sectionId) => {
  const blueprint = EXAM_BLUEPRINTS[sectionId];
  if (!blueprint) return [];

  const topics = [];
  blueprint.areas.forEach((area) => {
...
export const getTopicById = (topicId) => {
  const sectionId = topicId.split('-')[0];
 ...
 * @returns {boolean}
 */
export const isHR1Topic = (topicId) => {
 ...
 */
export const getProgressByArea = (userProgress, sectionId) => {
  const blueprint = EXAM_BLUEPRINTS[sectionId];
  if (!blueprint) return [];
