/**
 * EA Pass Guarantee Configuration
 * 
 * Defines the pass guarantee terms and requirements for the EA (Enrolled Agent) course.
...
export const EA_PASS_GUARANTEE: PassGuaranteeConfig = {
  enabled: true,
  name: 'EA Pass Guarantee',
  tagline: 'Pass All 3 Parts or Your Money Back',
  description: 'We\'re so confident in our EA prep course that we offer a full money-back guarantee. Complete the requirements below for each part and pass your exam, or receive a full refund for that part.',
  ...
  requirements: [
    ...
    {
      id: 'pg-req-005',
      title: 'Score 105+ on Final Mock Exam',
      description: 'Achieve a scaled score of 105 or above on the final practice exam (passing is 105)',
  ...
  refundPolicy: {
    type: 'full',
    percentage: 100,
    description: 'Full refund of your subscription payment if you meet all requirements but do not pass that part of the EA exam.',
    processingDays: 14,
  ...
  eligibilityWindow: {
    ...
    proofRequired: [
      'Prometric score report showing failing score (below 105)',
...
export function checkPassGuaranteeEligibility(
  ...
  for (const req of EA_PASS_GUARANTEE.requirements) {
...
export const PASS_GUARANTEE_SUMMARY = {
  ...
  bulletPoints: [
    ...
    'Score 105+ on final mock exam (IRS passing score)',
    'If you fail the real exam, we refund 100% of your payment',
    'No questions asked - just provide your Prometric score report'
  ],
  termsLink: '/terms/pass-guarantee',
  trustBadge: 'Thousands of students have passed the EA exam with VoraPrep'
};

// Part-specific tracking for EA (since there are 3 separate parts)
export interface EAPartProgress {
  part: 'SEE1' | 'SEE2' | 'SEE3';
  progress: UserProgress;
}

export function checkAllPartsEligibility(
  parts: EAPartProgress[]
): { 
  overallEligible: boolean; 
  partResults: { part: string; eligible: boolean; unmet: string[] }[] 
} {
  const partResults = parts.map(p => {
    const result = checkPassGuaranteeEligibility(p.progress);
    return {
      part: p.part,
  ...
  return {
    overallEligible: partResults.every(r => r.eligible),
    partResults
  };
}
