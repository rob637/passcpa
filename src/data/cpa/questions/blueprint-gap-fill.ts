...
export const ISC_V_GAP_QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    difficulty: 'medium',
    skillLevel: 'Application',
    question: 'An auditor discovers that an organization\'s financial reporting application maintains a detailed audit log of all user activities, including data modifications, report generation, and access attempts. However, the audit logs are stored on the same server as the application, and the application administrators have the ability to modify or delete log entries. What audit trail integrity concern exists?',
    options: [
      'Storing logs on the same server is efficient and does not create integrity concerns',
      'Audit logs stored on the same server and modifiable by application administrators cannot be relied upon as evidence — an administrator could modify logs to conceal unauthorized activities, undermining the integrity of the entire audit trail',
      'Application administrators would never modify audit logs',
      'Audit log integrity is only a concern for externally-facing systems',
    ],
    correctAnswer: 1,
    explanation: 'Audit trail integrity requires that logs be tamper-evident and protected from modification by the individuals whose activities they record. When administrators can modify logs: (1) the logs cannot serve as reliable evidence, (2) unauthorized activities can be concealed, (3) the detective control is compromised. Best practices include: centralized log collection (SIEM), write-once storage, integrity hashing, and separation of log management from system administration duties.',
    reference: 'AT-C 320',
  },
  {
    id: 'isc-gap-v-012',
