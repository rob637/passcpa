...
export const WORLD_CLASS_BATCH_28: Question[] = [
  ...
  },
  {
    ...
    difficulty: 'medium',
    skillLevel: 'Application',
    question: 'An organization processes credit card payments through its e-commerce platform and is subject to PCI DSS requirements. The auditor discovers that the organization stores full credit card numbers (PANs) in plaintext in the transaction database. The database is accessible to 25 customer service representatives who use it to look up transaction details. The organization\'s last PCI DSS assessment was 2 years ago. What compliance risk exists?',
    options: [
      'Storing plaintext PANs is acceptable for customer service purposes',
      'Storing plaintext PANs violates PCI DSS Requirement 3 (protect stored cardholder data) \u2014 the organization faces potential fines from card brands, increased transaction processing fees, loss of the ability to process card payments, and liability for any resulting data breach',
      'PCI DSS assessments are only required once every 5 years',
      'Customer service access to full PANs is a business necessity that overrides PCI DSS',
    ],
    correctAnswer: 1,
    explanation: 'PCI DSS requires that stored PANs be rendered unreadable (encryption, hashing, truncation, or tokenization). Plaintext PAN storage with broad access: (1) violates PCI DSS Requirement 3, (2) exposes the organization to card brand fines ($5,000-$100,000/month), (3) increases liability for any breach, (4) may result in termination of card processing privileges. The 2-year assessment gap suggests ongoing non-compliance has gone undetected.',
