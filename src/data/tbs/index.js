...
// TBS Types that mirror the actual CPA exam
...
• PV of $1: 0.74726
    ...
    hints: [
      ...
      'For finance leases, record both the ROU asset and lease liability at the present value of lease payments',
      'Interest expense = Beginning lease liability × Interest rate',
    ],
...
// Import additional TBS
import { ADDITIONAL_TBS, FAR_TBS_2, REG_TBS_2, AUD_TBS_2, BEC_TBS_2 } from './additional-tbs';

// Combine all TBS (original + additional)
export const FAR_TBS_ALL = [...FAR_TBS, ...FAR_TBS_2];
export const REG_TBS_ALL = [...REG_TBS, ...REG_TBS_2];
export const AUD_TBS_ALL = [...AUD_TBS, ...AUD_TBS_2];
export const BEC_TBS_ALL = [...BEC_TBS, ...BEC_TBS_2];

export const ALL_TBS = [...FAR_TBS_ALL, ...REG_TBS_ALL, ...AUD_TBS_ALL, ...BEC_TBS_ALL];

// Helper functions
export const getTBSBySection = (section) => {
  switch (section) {
    case 'FAR':
      return FAR_TBS_ALL;
    case 'REG':
...
export const getTBSByType = (type) => ALL_TBS.filter((tbs) => tbs.type === type);
...
export const getTBSStats = () => ({
  ...
  byType: Object.values(TBS_TYPES).reduce((acc, type) => {
